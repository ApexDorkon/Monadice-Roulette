...
/* ---------------------------------------------------------
   ...
   - Added campaign whitelist controlled by controller/owner
   - Added ReentrancyGuard
   - Added basic number-range validation (0..37)
--------------------------------------------------------- */
contract RouletteNFT is ReentrancyGuard {
    enum BetType {
    ...
    /* ---------------------------------------------------------
       CAMPAIGN WHITELIST (O(1) checks)
       - whitelistedCampaigns array is for off-chain enumeration
--------------------------------------------------------- */
    ...
    /* ---------------------------------------------------------
       ...
       (Only controller/owner can modify)
    --------------------------------------------------------- */
    ...
    function placeBet(
        ...
        require(campaign != address(0), "campaign=0");
        // ✅ Added: whitelist enforcement (put here, before any external calls)
        ...
        nonReentrant
    {
        ...
        // ✅ Slight hardening: deterministic uniqueness, avoids timestamp coupling
    ...
    /* ---------------------------------------------------------
       UTILITIES (unchanged, plus number-range validation)
    --------------------------------------------------------- */
    function _validateBetInput(
        ...
        // ✅ Added: each number must be 0..37 (37 = double zero)
        for (uint256 i = 0; i < numbers.length; i++) {
